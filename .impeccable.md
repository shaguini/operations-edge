...
- **Typography**: Avoid the current Outfit font (reflex default). Needs a pairing that feels like a reliable trade publication — a sturdy, slightly characterful sans for UI/body, and a display face with some editorial weight for headings. Monospace for code-adjacent labels or ratings only, not as a general "technical" shorthand.
